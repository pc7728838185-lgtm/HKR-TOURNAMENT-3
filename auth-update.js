...
function toast(m){
  ...
  window.__hkrT = setTimeout(() => t.style.display = "none", 2600);
}
...
function login(){
  modal(`
    ...
    <div class="auth-actions">
      <button class="auth-link" id="forgot">Forgot password?</button>
      <button class="auth-link" id="signup">Create a new account</button>
    </div>
  `);

  $("#hkrLogin").onsubmit = async e => {
    e.preventDefault();
    ...
    if(window.loadSession) window.loadSession();
  };
...
function signup(){
  modal(`
    ...
    <div class="auth-actions">
      <button class="auth-link" id="back">
  ...
  $("#hkrSignup").onsubmit = async e => {
    e.preventDefault();

    const s = client();
    if(!s) return;

    const referId = $("#refer_id").value.trim();
    ...
    /* If session is immediately available, save profile data */
...
function install(){
  ...
  s.auth.onAuthStateChange(event => {
    ...
    if(event === "PASSWORD_RECOVERY"){
      ...
      setTimeout(
        () => window.loadSession && window.loadSession(),
        0
      );
    }
  });
